  ...
  return (
    <div style={rootStyle}>
      ...
      <div ref={wrapRef} style={{ position:"relative", padding:"0 16px 24px" }}>
        <div style={grid}>
          {people.map(p => (
            <div key={p.id} data-sim-id={p.id} style={card} title={p.label ?? p.id}>
              <div style={banner}>
              ...
              <div style={body}>
                {Array.isArray(p.traits) && p.traits.length > 0 && (
                  <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                    {p.traits.slice(0,6).map((t,i)=><span key={i} style={{ padding:"2px 8px", borderRadius:999, border:"1px solid #cfeede", background:"#fff", fontSize:12 }}>{t}</span>)}
                  </div>
                )}
        ...
        {/* Kontextmenü Card */}
        {cardCtx.open && (
          <div ref={cardMenuRef} style={{ ...menuBox, left: 0, top: 0 }} onContextMenu={(e)=>e.preventDefault()}>
            <div style={menuItem} onClick={()=>{ onOpenInfo?.(cardCtx.id); setCardCtx({ open:false, x:0, y:0, id:"" }); }}>
              ℹ️ Infos bearbeiten…
            </div>
            <div style={menuItem} onClick={()=>{ openImageDialogForSim(cardCtx.id); setCardCtx({ open:false, x:0, y:0, id:"" }); }}>
              🖼️ Bild ändern
            </div>
            <div style={menuItem} onClick={()=>{ onFocusInExplorer?.(cardCtx.id); setCardCtx({ open:false, x:0, y:0, id:"" }); }}>
              🗺️ Im Explorer fokussieren
            </div>
            <div
              style={{ ...menuItem, borderBottom:0, color:"#b3261e" }}
              onClick={()=>{
                const nextNodes = nodes.filter(n => n.id !== cardCtx.id);
                const nextEdges = edges.filter(e => e.source !== cardCtx.id && e.target !== cardCtx.id);
        ...
        {bgCtx.open && (
          <div ref={bgMenuRef} style={{ ...menuBox, left: 0, top: 0 }} onContextMenu={(e)=>e.preventDefault()}>
            <div style={menuItem} onClick={()=>{ onRequestCreateSim?.(); setBgCtx({ open:false, x:0, y:0 }); }}>
              ➕ Neuen Sim anlegen…
            </div>
            ...
            <div style={{ ...menuItem, borderBottom:0, color:"#b3261e" }} onClick={()=>{ onBgClear?.(); setBgCtx({ open:false, x:0, y:0 }); }}>
      ...
      {/* Bild-Auswahl-Modal – Buttons jetzt immer sichtbar */}
      {imgDlg.open && (
        <div
          ...
          >
              ...
              <div style={{ display:"grid", gap:12 }}>
                ...
                {imgDlg.preview ? (
                  <img
                    src={imgDlg.preview}
                    alt="Vorschau"
                    style={{ width:"100%", maxHeight:"60vh", objectFit:"cover", borderRadius:12, border:"1px solid #e5e7eb" }}
                  />
                ) : (
            ...
            <div
              style={{
                ...
                paddingTop:12, borderTop:"1px solid #e5e7eb", background:"#fff",
